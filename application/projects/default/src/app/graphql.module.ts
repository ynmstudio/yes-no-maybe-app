...
import { Auth, getIdTokenResult, idToken } from '@angular/fire/auth';
...
const authCtx = (auth: Auth) =>
  setContext(async (_, { headers }) => {
    // Grab token if there is one in storage or hasn't expired

    const token = await firstValueFrom(idToken(auth));
...
export function provideGraphQL(): Provider[] {
  return [
    Apollo,
    {
      ...
      deps: [HttpLink, Auth, ConnectionService, AlertService]
    }];
}
